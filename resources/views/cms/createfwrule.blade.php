        ...
        <div class="card-body" id="action">
            <div class="form-group">
                ...
                <select class="form-control"
                        name="action"
                        required>
                    <option value="" disabled>Select one of the following actions</option>
...
<script type="text/javascript">
    function sendToCloudflare() {
        ...
        if (!zones || !description || !action.length || !expression) {
            alert('You must fill in all required inputs');
            ...
            let element = '<div class="card-body" id="products">' + 
                '<div class="form-group">' +
                    ...
                    '<select class="form-control" name="products[]" multiple required>' +
                        ...
                        '<option value="uaBlock" {{ Arr::has(old('products'), 'uaBlock') ? 'selected' : '' }}>User Agent Block</option>' +
                        '<option value="bic" {{ Arr::has(old('products'), 'bic') ? 'selected' : '' }}>Browser Integrity Check</option>' +
                        '<option value="hot" {{ Arr::has(old('products'), 'hot') ? 'selected' : '' }}>Hotlink Protection</option>' +
                        '<option value="securityLevel" {{ Arr::has(old('products'), 'securityLevel') ? 'selected' : '' }}>Security Level</option>' +
                        '<option value="rateLimit" {{ Arr::has(old('products'), 'rateLimit') ? 'selected' : '' }}>Rate Limit</option>' +
                        '<option value="waf" {{ Arr::has(old('products'), 'waf') ? 'selected' : '' }}>WAF Managed Rules</option>' +
                        '<option value="zoneLockdown" {{ Arr::has(old('products'), 'zoneLockdown') ? 'selected' : '' }}>Zone Lockdown</option>' +
                    '</select>' +
                '</div>' +
            '</div>';
